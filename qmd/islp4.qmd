...
![Medical conditions coding](figures/coding.png){fig-align="center"}
:::
...
![The Default data set](figures/4_1.png){fig-align="center" width="800"}
...
![Classification using the Default data](figures/4_2.png){fig-align="center" width="800"}
...
-   Equivalently, a one-unit increase in X *multiplies the odds* by e<sup>β₁</sup>.
...
- Interpretation:  βⱼ represents the change in log-odds for a one-unit increase in Xⱼ, *holding all other predictors constant*.
...
![Confounding in the Default data.](figures/4_3.png)
...
- Let π<sub>k</sub> be the *prior probability* that an observation comes from class *k*.
...
- To use this, we need to estimate π<sub>k</sub> and f<sub>k</sub>(x). Estimating π<sub>k</sub> is usually easy (fraction of training observations in class k). Estimating f<sub>k</sub>(x) is harder.
...
- In practice, we don't know the true parameters (π<sub>k</sub>, μ<sub>k</sub>, σ²).  We *estimate* them from the training data.

## LDA: Estimating the Parameters

-   μ̂<sub>k</sub> = (1/n<sub>k</sub>) Σ<sub>i:yᵢ=k</sub> xᵢ  (sample mean for class k)
-   σ̂² = (1/(n-K)) Σ<sub>k=1</sub><sup>K</sup> Σ<sub>i:yᵢ=k</sub> (xᵢ - μ̂<sub>k</sub>)²  (pooled variance estimate)
-   π̂<sub>k</sub> = n<sub>k</sub>/n (sample proportion for class k)

- We plug these estimates into the discriminant function.

## LDA: Example

![One-dimensional normal density functions and LDA decision boundary](figures/4_4.png)
...
- We assume X = (X₁, X₂, ..., X<sub>p</sub>) follows a *multivariate Gaussian* distribution, with a class-specific mean vector and a *common covariance matrix*.
...
- We estimate the parameters (μ<sub>k</sub>, Σ, π<sub>k</sub>) from the training data.

## LDA: Example with Three Classes

![Multivariate Gaussian distribution with three classes, and LDA decision boundaries](figures/4_6.png)
::: {.callout-note appearance="simple"}
...
![Error rates as a function of threshold](figures/4_7.png)
...
![ROC curve for LDA classifier on Default data](figures/4_8.png)
...
-   Each class has its *own* covariance matrix, Σ<sub>k</sub>.
...
- We estimate the parameters (μ<sub>k</sub>, Σ<sub>k</sub>, π<sub>k</sub>) from the training data.
...
![LDA and QDA decision boundaries](figures/4_9.png)
...
- f<sub>k</sub>(x) = f<sub>k1</sub>(x₁) × f<sub>k2</sub>(x₂) × ... × f<sub>kp</sub>(x<sub>p</sub>)
...
- If X<sub>j</sub> is *quantitative*:
    - We can assume X<sub>j</sub> | Y = k ~ N(μ<sub>jk</sub>, σ<sub>j</sub>²) (Gaussian).
...
## Naive Bayes: Example
::: {layout-ncol=3}
![f11](figures/4_10_1.png)

![f12](figures/4_10_2.png)

![f13](figures/4_10_3.png)
:::

::: {layout-ncol=3}
![f21](figures/4_10_4.png)

![f22](figures/4_10_5.png)

![f23](figures/4_10_6.png)
:::
...
| :---------------- | :---------------------------------------------------------------------------------------------------------------------------------------------------- |
| Logistic Regression | β<sub>k0</sub> + Σ<sub>j=1</sub><sup>p</sup> β<sub>kj</sub>x<sub>j</sub>  (linear)                                                                                                 |
| LDA               | a<sub>k</sub> + Σ<sub>j=1</sub><sup>p</sup> b<sub>kj</sub>x<sub>j</sub> (linear)                                                                                                  |
| QDA               | a<sub>k</sub> + Σ<sub>j=1</sub><sup>p</sup> b<sub>kj</sub>x<sub>j</sub> + Σ<sub>j=1</sub><sup>p</sup> Σ<sub>l=1</sub><sup>p</sup> c<sub>kj</sub><sub>l</sub>x<sub>j</sub>x<sub>l</sub> (quadratic)                        |
| Naive Bayes      | a<sub>k</sub> + Σ<sub>j=1</sub><sup>p</sup> g<sub>kj</sub>(x<sub>j</sub>) (additive, where g<sub>kj</sub> is a function of x<sub>j</sub>)                                                          |
...
## A comparison of classification methods：An empirical comparison
...
::: {layout-ncol=2}
::: half-width
![FIGURE 4.11](figures/4_11.png)
:::

::: half-width
![FIGURE 4.12](figures/4_12.png)
:::
:::
...
- Three types of regression models have been discussed: linear, logistic and Poisson. They share some common characteristics:
    ...
    2.  Modeling the mean of Y.
        Use different *link function* η, these regression models could be expressed as below:
